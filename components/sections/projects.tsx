...
const projects: Project[] = [
  {
    ...
    description:
      "A working prototype of a new negotiation primitive, built for high-stakes private deals where neither side wants to reveal their cards first (M&A, regulatory cooperation, AI safety agreements). Designed and shipped a four-agent Claude orchestration: two Negotiator AIs talk inside a sealed environment, a Mediator polices for jailbreaks and prompt-injection attacks, and a Synthesizer extracts joint proposals as structured tool calls so sensitive content never leaks to humans. The privacy guarantee is structural, because the synthesizer's tool schema has no prose field, so the AI cannot put fact contents into any human-facing output. Built end-to-end: FastAPI + SQLAlchemy backend, React + Vite frontend, asymmetric server-side rendering, audit endpoints, and a smoke test that verifies the mediator catches injection attacks at high severity.",
  ...
  },
  {
    ...
    tagline:
      "Zoom + n8n + OpenAI + HubSpot | Transcripts to Structured Insights to CRM | Built for SDR Adoption",
    description:
  ...
  {
    title: "Bronze to Gold Data Warehouse: AI-Ready Infrastructure",
    tagline:
    ...
    <section id="projects" className="bg-black px-6 md:px-16 lg:px-32 py-20">
      ...
      <div className="mx-auto grid max-w-5xl gap-6 sm:grid-cols-2 lg:grid-cols-2">
        {projects.map((p) => (
          <article
            key={p.title}
            className="group relative overflow-hidden rounded-2xl border border-white/10 bg-black/40 transition-transform duration-200 hover:-translate-y-1 shadow-[0_0_0_0_rgba(0,0,0,0)] hover:shadow-[0_20px_40px_-20px_rgba(0,0,0,0.6)] flex flex-col h-full"
          >
            {p.image ? (
